...
class HotelService {
  ...
  async getDashboard(): Promise<DashboardData> {
    const r = await api.get(`/api/hotel-bookings/${this.code}/dashboard`);
  ...
  async getRooms(floor?: string, roomType?: string): Promise<RoomDetail[]> {
    ...
    const r = await api.get(`/api/hotel-property/${this.code}/rooms?${params}`);
    return r.data?.data ?? r.data ?? [];
  }

  async getRoomMap() {
    const r = await api.get(`/api/hotel-rooms/${this.code}/map`);
    return r.data?.data ?? r.data;
  }

  async getRoomAvailability(checkIn: string, checkOut: string) {
    const r = await api.get(`/api/hotel-rooms/${this.code}/availability?checkIn=${checkIn}&checkOut=${checkOut}`);
    return r.data?.data ?? r.data;
  }

  async getRoomForecast(from: string, to: string) {
    const r = await api.get(`/api/hotel-rooms/${this.code}/forecast?from=${from}&to=${to}`);
    return r.data?.data ?? r.data ?? [];
  }

  async updateRoomStatus(roomNo: string, status: string, cleanDirty?: number) {
    const r = await api.patch(`/api/hotel-rooms/${this.code}/rooms/${roomNo}/status`, { status, cleanDirty });
    return r.data;
  }
  ...
  async getBookings(params: Record<string, string | number>): Promise<{ items: BookingDto[]; total: number }> {
    ...
    const r = await api.get(`/api/hotel-bookings?${qs}`);
    const d = r.data?.data ?? r.data;
    return { items: d?.items ?? d ?? [], total: d?.total ?? 0 };
  }

  async getBookingById(id: number): Promise<BookingDto> {
    const r = await api.get(`/api/hotel-bookings/${id}`);
    return r.data?.data ?? r.data;
  }

  async createBooking(req: CreateBookingRequest): Promise<BookingDto> {
    const r = await api.post('/api/hotel-bookings', { ...req, hotelCode: this.code });
    return r.data?.data ?? r.data;
  }

  async updateBookingStatus(id: number, status: string, paidAmount?: number, cancelReason?: string) {
    const r = await api.patch(`/api/hotel-bookings/${id}/status`, { bookingId: id, status, paidAmount, cancelReason });
    return r.data;
  }

  async generateInvoice(id: number, paymentMethod: string) {
    const r = await api.post(`/api/hotel-bookings/${id}/invoice`, { paymentMethod });
    return r.data?.data ?? r.data;
  }
  ...
  async getVehicles(status?: string): Promise<VehicleDto[]> {
    ...
    const r = await api.get(`/api/hotel-vehicles/${this.code}${qs}`);
    return r.data?.data ?? r.data ?? [];
  }

  async getActiveRentals(): Promise<VehicleRentalDto[]> {
    const r = await api.get(`/api/hotel-vehicles/${this.code}/rentals/active`);
    return r.data?.data ?? r.data ?? [];
  }

  async createRental(req: object): Promise<VehicleRentalDto> {
    const r = await api.post('/api/hotel-vehicles/rentals', { ...req, hotelCode: this.code });
    return r.data?.data ?? r.data;
  }

  async returnVehicle(rentalId: number, req: object) {
    const r = await api.post(`/api/hotel-vehicles/rentals/${rentalId}/return`, req);
    return r.data;
  }

  async upsertVehicle(data: object) {
    const r = await api.post('/api/hotel-vehicles', { ...data, hotelCode: this.code });
    return r.data?.data ?? r.data;
  }
  ...
  async getTours(tourType?: string): Promise<HotelTourDto[]> {
    ...
    const r = await api.get(`/api/hotel-tours/${this.code}${qs}`);
    return r.data?.data ?? r.data ?? [];
  }

  async upsertTour(data: object) {
    const r = await api.post('/api/hotel-tours', { ...data, hotelCode: this.code });
    return r.data?.data ?? r.data;
  }

  async deleteTour(id: number) {
    await api.delete(`/api/hotel-tours/${id}`);
  }

  async getGuides() {
    const r = await api.get(`/api/hotel-tours/${this.code}/guides`);
    return r.data?.data ?? r.data ?? [];
  }

  async upsertGuide(data: object) {
    const r = await api.post(`/api/hotel-tours/${this.code}/guides`, { ...data, hotelCode: this.code });
    return r.data?.data ?? r.data;
  }
  ...
  async getTourSchedules(tourCode?: string, from?: string, to?: string) {
    ...
    const r = await api.get(`/api/hotel-tours/${this.code}/schedules?${qs}`);
    return r.data?.data ?? r.data ?? [];
  }

  async upsertSchedule(data: object) {
    const r = await api.post(`/api/hotel-tours/${this.code}/schedules`, { ...data, hotelCode: this.code });
    return r.data?.data ?? r.data;
  }
  ...
  async getGuests(search?: string): Promise<HotelGuest[]> {
    ...
    const r = await api.get(`/api/hotel-guests/${this.code}${qs}`);
    return r.data?.data ?? r.data ?? [];
  }

  async getGuestByPhone(phone: string): Promise<HotelGuest | null> {
    try {
      const r = await api.get(`/api/hotel-guests/${this.code}/by-phone/${phone}`);
      return r.data?.data ?? r.data;
    } catch { return null; }
  }
  ...
  async getServices(category?: string): Promise<HotelService[]> {
    ...
    const r = await api.get(`/api/hotel-services-catalog/${this.code}${qs}`);
    return r.data?.data ?? r.data ?? [];
  }

  async upsertService(data: object) {
    const r = await api.post('/api/hotel-services-catalog', { ...data, hotelCode: this.code });
    return r.data?.data ?? r.data;
  }

  async deleteService(id: number) {
    await api.delete(`/api/hotel-services-catalog/${id}`);
  }

  // Reports
  async getOccupancyReport(from: string, to: string) {
    const r = await api.get(`/api/hotel-reports/${this.code}/occupancy?from=${from}&to=${to}`);
    return r.data?.data ?? r.data ?? [];
  }

  async getRevenueByMonth(year?: number) {
    const y = year || new Date().getFullYear();
    const r = await api.get(`/api/hotel-reports/${this.code}/revenue/monthly?year=${y}`);
    return r.data?.data ?? r.data ?? [];
  }

  async getRevenueToday() {
    const r = await api.get(`/api/hotel-reports/${this.code}/revenue/today`);
    return r.data?.data ?? r.data;
  }

  async getVehicleUtilization(from: string, to: string) {
    const r = await api.get(`/api/hotel-reports/${this.code}/vehicles/utilization?from=${from}&to=${to}`);
    return r.data?.data ?? r.data ?? [];
  }

  // Room Types
  async getRoomTypes() {
    const r = await api.get(`/api/hotel-property/${this.code}/room-types`);
    return r.data?.data ?? r.data ?? [];
  }

  // Settings
  async getSettings() {
    const r = await api.get(`/api/hotel-property/${this.code}/settings`);
    return r.data?.data ?? r.data ?? [];
  }

  async upsertSetting(key: string, value: string, description?: string) {
    const r = await api.post(`/api/hotel-property/${this.code}/settings`, { key, value, description });
    return r.data;
  }

  // Group members
  async getGroupMembers(bookingId: number) {
    const r = await api.get(`/api/hotel-tours/bookings/${bookingId}/members`);
    return r.data?.data ?? r.data ?? [];
  }

  async addGroupMember(bookingId: number, data: object) {
    const r = await api.post(`/api/hotel-tours/bookings/${bookingId}/members`, { ...data, hotelCode: this.code, bookingId });
    return r.data?.data ?? r.data;
  }
}
