...
export type CarFormValues = {
  ...
  fuelAmount?: number;
  content?: string;
  files?: string[];           // list URL gửi lên BE
...
export const CarDialog = ({
  ...
  // Danh sách URL ảnh (cả cũ + mới, gộp chung)
  ...
  // Load ảnh cũ khi dialog mở
  useEffect(() => {
    if (!open) return;
    const existing = (defaultValues?.file ?? [])
      .map((f) => f.fileUrl ?? "")
      .filter(Boolean);
  ...
  const addUrl = () => {
    ...
    try {
      new URL(trimmed); // validate URL hợp lệ
    } catch {
      ...
      <DialogTitle sx={{ pb: 1 }}>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
          {isEditMode
            ...
            : "Điền thông tin để thêm xe vào hệ thống"}
        </Typography>
      </DialogTitle>
        ...
        <Stack spacing={2.5}>
          ...
          <Controller
            ...
            rules={{
              ...
              pattern: {
                value: LICENSE_PLATE_REGEX,
                message: "Biển số không hợp lệ. Ví dụ: 51F-12345",
              },
            }}
            render={({ field }) => (
              <TextField
          ...
          <Box>
            ...
            <Stack direction="row" spacing={1} alignItems="flex-start">
              ...
              <Button
                ...
                onClick={addUrl}
                disabled={loading}
                sx={{ mt: "1px", whiteSpace: "nowrap", minWidth: 80 }}
              >
                Thêm
              </Button>
            </Stack>
